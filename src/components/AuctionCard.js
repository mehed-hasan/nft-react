    ...
    return ( 
        <div className="nft_card -50 shadow">
              ...
              <div className="nft_counter text-center theme-text-color ">
                <p>Time Remaining</p>
                <h4>{data.timeRemained}D : 23H : 45M : 40S</h4>
              </div>
  
              <div className="nft_image theme-text-color">
                <Link to={`/details/${data.id}`}><img className="img-fluid thumb" style={{height:'300px'}} src={`${require(`../images/banner/${data.image}`)}`} alt="error" /></Link>
                ...
                <Link to="#" className="product_title theme-text-color">{data.name}</Link>
                <p>Highest bid 1/12</p>
                <div className="price_box d-flex justify-content-between">
                  <p className=""><b>{data.price}ETH</b></p>
                  <p className="price">{data.loved} <span>
